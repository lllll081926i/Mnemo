...
Mnemo's macOS embedded MPV route expects native assets under:
...
```text
engine/darwin/<arch>/mpv-texture/mnemo-mpv-texture.node
```

Notes:

- `<arch>` is `arm64` first; `x64` must be added only after a separate verification pass.
- `mnemo-mpv-texture.node` must export `mpvTexture`.
...
Use `native/mnemo-mpv-texture/scripts/bundle-dylibs-mac.sh` to prepare this directory from a local native addon build and local Homebrew/libmpv installation. The script does not download binaries.
