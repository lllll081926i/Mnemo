...
describe('deep layout shell port', () => {
  ...
  it('keeps background execution only for the hidden transfer workers', () => {
    ...
    expect(windowSource).toContain('nodeIntegrationInWorker: false')
  ...
  it('starts the upload worker only when a message needs it and preserves the first message', () => {
    ...
    expect(rendererEntry).toContain('window.WebUploadPort.send(event)')
    expect(rendererEntry).toContain('window.WebUploadPort.onMessage')
    expect(rendererEntry).toContain('window.WebOnSetPage')
    expect(rendererEntry).toContain('window.WebOnSetTheme')
    expect(workerEntry).toContain('window.WebWorkerPort.onMessage')
    expect(workerEntry).toContain('window.WinMsgToMain')
    expect(read('src/workerpage/workercmd.ts')).toContain('window.WebWorkerPort.notifyReady()')
  })
