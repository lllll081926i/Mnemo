...
describe('deep layout shell port', () => {
  ...
  it('loads streaming and ASS playback runtimes only for the media that requires them', () => {
    ...
    expect(video).toContain('function isMpegTsQuality(item: VideoQuality)')
    expect(video).toContain('const nonTs = qualities.filter((item) => !isMpegTsQuality(item))')
    expect(video).toContain('const bestTranscode = pool.find((item) => item.quality !== \'Origin\')')
  ...
  it('keeps all setting groups in one scrollable document with sidebar anchors', () => {
    ...
    expect(setting).not.toContain("key: 'aliyun'")
  ...
  it('starts the packaged window without global TLS bypasses or remote icon fonts', () => {
    ...
    expect(read('electron/main/core/ipcEvent.ts')).toContain('icon: loadAppNativeImage()')
  ...
  it('sets the Mnemo identity before creating Windows windows and shortcuts', () => {
    ...
    expect(windowSource).toContain('loadAppNativeImage()')
    expect(windowSource).toContain('loadTrayNativeImage()')
    expect(windowSource).toContain('showOrCreateMainWindow')
    expect(read('electron/main/utils/appIcon.ts')).toContain('export function loadAppNativeImage')
    expect(read('electron/main/utils/appIcon.ts')).toContain('export function loadTrayNativeImage')
    expect(read('electron/main/utils/appIcon.ts')).toContain('EMBEDDED_TRAY_PNG_32_BASE64')
    expect(read('electron/main/core/ipcEvent.ts')).toContain('icon: loadAppNativeImage()')
    expect(read('electron-builder.json')).toContain('"productName": "Mnemo"')
    expect(read('electron-builder.json')).toContain('"executableName": "Mnemo"')
    expect(read('electron-builder.json')).toContain('"buildResources": "build"')
    expect(read('electron-builder.json')).toContain('"shortcutName": "Mnemo"')
