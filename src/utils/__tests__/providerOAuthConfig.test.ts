...
describe('provider OAuth runtime configuration', () => {
  ...
  it('routes PikPak visual challenges through a sandboxed Mnemo window', () => {
    ...
    expect(mainIpc).toContain('show: true')
    expect(mainIpc).toContain('sandbox: true')
    expect(mainIpc).toContain('devTools: false')
    expect(mainIpc).toContain("url.pathname === '/credit/v1/report'")
    expect(mainIpc).toContain("method === 'Network.responseReceived'")
    expect(mainIpc).toContain("event.sender.send('PikPakCaptcha:completed', { captchaToken })")
    expect(login).toContain('loginPikPakWithCaptcha')
    expect(login).toContain('loginPikPakWithVerifiedCaptcha')
    expect(login).toContain('pikpakAutoLoginQueued')
    expect(login).toContain('queuePikPakCaptchaLogin()')
    expect(login).toContain('pikpak_login_cooldown_until')
    expect(login).toContain('PikPak 暂时限制了登录请求，请等待 {{ pikpakCooldownSeconds }} 秒后再试')
    expect(login).toContain(':disabled="pikpakCooldownSeconds > 0"')
    expect(auth).toContain('PIKPAK_MIN_LOGIN_COOLDOWN_SECONDS = 30')
