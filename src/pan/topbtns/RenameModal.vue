...
<template>
  <a-modal :visible='visible' modal-class='modalclass' :footer='false' :unmount-on-close='true' :mask-closable='false'
    ...
    <div class="mn-modal">
      <div v-if='form.bakName' class='mn-panel' :title='form.bakName'>
        <span class='mn-panel-label'>当前名称</span>
        <span class='mn-panel-value'>{{ form.bakName }}</span>
      </div>
      <a-form ref='formRef' :model='form' layout='vertical' class='mn-form'>
        <a-form-item field='fileName' :rules='ispic ? album_rules : file_rules' :label="ispic ? '相册名' : '新名称'">
          <a-input v-model='form.fileName' :placeholder='form.bakName' allow-clear
                   :input-attrs="{ id: 'RenameInput', autofocus: 'autofocus' }" />
        </a-form-item>
        <a-form-item v-if='ispic' field='description' label='相册描述'>
