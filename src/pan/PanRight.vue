...
import { dropMoveSelectedFile, handleUpload, menuCopySelectedFile, menuCreatShare, menuTrashSelectFile, topSearchAll, uploadLocalPaths } from './topbtns/topbtn'
import { modalCreatNewDir, modalPassword, modalRename, modalSelectPanDir, modalCloudOfflineDownload } from '../utils/modal'
...
const { capabilities: currentProviderCapabilities } = useCurrentDriveProvider()
...
const handleOfflineDownload = () => {
  ...
  const isRoot = !panfileStore.DirID || isDriveProviderRootId('pikpak', panfileStore.DirID)
...
keyboardStore.$subscribe((_m: any, state: KeyboardState) => {
  ...
  if (
    TestCtrlShift('f', state.KeyDownEvent, () => {
      if (!currentProviderCapabilities.value.search) {
        message.error('当前网盘不支持全盘搜索')
        return
      }
  ...
  if (TestKey('f2', state.KeyDownEvent, () => currentProviderCapabilities.value.rename && modalRename(false, panfileStore.IsListSelectedMulti, false))) return
...
const onPanDrop = (e: any) => {
  ...
  if (filesList && filesList.length > 0) {
    ...
    void uploadLocalPaths(files, panfileStore.DirID)
  }
}
