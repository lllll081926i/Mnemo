...
export const buildOneDriveAuthUrl = async (clientId: string, verifier: string, state: string, redirectUri: string): Promise<string> => {
...
const applyOneDriveAccount = async (token: ITokenInfo) => {
  ...
  if (account) {
    ...
    token.user_id = buildDriveProviderUserId('onedrive', accountId)
  ...
  if (drive) {
    token.default_drive_id = buildDriveProviderDriveId('onedrive', drive.id || token.user_id)
...
const normalizeOneDriveToken = (data: any, clientId: string): ITokenInfo | null => {
  ...
  token.user_id = buildDriveProviderUserId('onedrive', hashString(data.refresh_token || data.access_token))
...
export const refreshOneDriveAccessToken = async (token: ITokenInfo): Promise<ITokenInfo | null> => {
  // 单飞：同一账号并发的令牌刷新只发一次请求，其余复用同一个 Promise，避免 refresh_token 被多次消费导致失效。
  ...
  const promise = (async (): Promise<ITokenInfo | null> => {
    try {
      ...
      token.default_drive_id = token.default_drive_id || buildDriveProviderDriveId('onedrive', token.user_id)
      await applyOneDriveAccount(token)
      return token
    } finally {
