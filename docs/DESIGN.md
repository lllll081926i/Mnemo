...
- 视频统一由随包 mpv 独立窗口播放，不回退到 HTML5 或系统播放器；前端为**悬浮式控制条**（底部居中，≤520px）：标题行 + 5px 进度条（点击跳转）+ 控制行（快退/播放暂停/快进/时间/倍速/音量）。
- 进度为本地估算（ticker × 倍速，seek 校正）；起播时按 `GetPlayCursor` 续播，关闭时 `SavePlayCursor`。
