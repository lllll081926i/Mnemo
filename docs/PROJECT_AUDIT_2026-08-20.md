...
| P2-02 | 在线文本编辑把“加入上传队列”当成“已保存” | `frontend/src/components/PreviewModal.vue:260-275`、`internal/app/drive.go:20-43`。临时文件按 basename 共用，可能互相覆盖并残留。应使用独立临时目录，等待上传完成事件后才提示成功，失败可重试且自动清理。 |
| P2-03 | 目录缓存没有 TTL/版本，超大目录没有虚拟滚动 | 长时间运行会展示陈旧目录；大目录渲染和内存压力随文件数线性增长。应增加时间戳、失效事件、容量上限和虚拟列表。 |
