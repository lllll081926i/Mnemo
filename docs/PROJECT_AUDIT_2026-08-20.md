...
前端已接入 Vitest：除账号/provider 解析、能力映射、容量格式化和文件打开类型等纯逻辑外，现有 12 个 jsdom 测试覆盖 Modal 的 Esc/焦点恢复、UiSelect 的键盘跳过禁用项、选择目录弹窗的业务错误提示、WebDAV 预设自动填充、S3 写入/内网预览默认关闭、容量刷新低频保护，并以 mock Wails API 覆盖目录缓存 RPC 串行队列。尚无完整页面级 E2E；Vite 构建和这些组件测试仍不能替代真实账号切换、上传冲突、设置生效和 Wails 桥接端到端验证。
...
| 已完成（本轮补充） | 前端 mock Wails 关键功能测试：WebDAV 模板自动填充、S3 安全默认值、容量刷新低频保护 |
