...
| P2-02（已关闭主要路径） | 在线文本编辑把“加入上传队列”当成“已保存” | 已改为每次保存使用独立临时目录，提示语明确为“已加入上传队列”；上传成功后仅清理应用创建且位于受控 `mnemo_edit_*` 临时目录中的源文件，失败、暂停或普通用户文件均保留。大目录渲染窗口已接入且不改变现有视觉样式；真正等待远端完成仍作为下一项后端可靠性增强。 |
...
- 在线文本编辑已改为准确提示“加入上传队列”，下一步将让 Wails 保存调用等待上传任务进入终态。目录缓存和大目录虚拟列表主要路径已完成；组件级测试等剩余事项见 P2。
...
| 体验迭代 | 设置项生效、文本编辑真完成、缓存/错误状态/键盘操作；大目录虚拟列表已完成，大组件拆分按需求暂缓 |
