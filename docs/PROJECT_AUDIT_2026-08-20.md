...
| F-39 | 原子 JSON 写入重命名失败会遗留临时文件，上传会话文件权限偏宽 | 常规 JSON、账号凭据和上传会话在写入或重命名失败后都会清理 `.tmp`，不覆盖原目标；上传会话落盘权限改为 `0600`。新增“目标被目录占用”故障注入测试，确认失败不改动目标且不留临时文件。 |
