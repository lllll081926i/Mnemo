...
| 子功能 | 状态 | Go 证据 | 差距 |
|--------|:----:|---------|------|
| 整包上传 | ✅ | `upload.go` html5up.php；V0/V1/V2/V3 分别限制 100/200/300/550MiB，worker 会复核 | 无 |
| 格式拦截 | ✅ | `upload.go` 以蓝奏公布的后缀白名单在选文件、文件夹、拖拽和 worker 四处拦截 | 无 |
| 分片上传 | ❌ | 接口限制 | 无（设计） |
| 秒传 | ❌ | 接口限制 | 无（设计） |

> 登录时选择当前会员等级；旧账号的历史凭据按 V0 保守处理。普通蓝奏按产品规则展示“无限空间”，不为容量额外请求接口。
