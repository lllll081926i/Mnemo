...
- 后续审计：移除 Nextcloud 与空下载工作窗，补齐 WebDAV / S3 / GoFile / Google Drive 基础路径测试，并将 Aria2 改为首次下载时启动
...
`aliyun` · `139` · `189` · `guangya` · `pikpak` · `quark` · `onedrive` · `dropbox` · `gdrive` · `gofile` · `webdav` · `s3` ·（及 `unknown`）
...
- 挂载存储（WebDAV / S3）：连接层有，与 pan UI 深度需联调。
...
- 近两日修复：aria2 启动竞态、Node 25 相关警告、首次下载时懒启动引擎
...
- 图书、AI、RSS、DLNA、Reedy 等旧目录已无受管文件与生产引用
- `PageMusic.vue` 仅保留本地音频预览和歌词，符合当前产品范围
...
6. **WebDAV / S3**：与 pan 菜单、上传队列、预览鉴权全路径验收

### P2 — 产品洁癖与发布

7. 无入口代码审计已完成：`src/rss/**`、`layout/book-*`、`aisearch`、`reedy`、DLNA 等不再有受管文件或生产引用
...
| 路径 | 说明 |
|------|------|
| `src/rss/**`、`src/layout/aisearch`、`book-*`、`electron/main/reedy/**` | 已无受管文件与生产引用 |
| `src/module/dlnacast`、`movie-db`、旧 WebDAV server | 已移除；不再进入依赖图 |
| `src/module/video-plugins/artplayer-plugin-libass` | 已移除，保留 npm `artplayer-plugin-jassub` |
| `static/crx` | 仅 dev 加载扩展 |
| `PageMusic` + `musicMetadata` | 本地音频预览与歌词，按当前产品范围保留 |
...
Electron Main
  ...
  ├─ MotrixApplication → 仅首次下载或启动恢复任务时拉起 aria2c
  ├─ Tray / 自动更新
  │
...
| 优先级 | 原因 | 说明 |
|--------|------|------|
| **1** | **上传窗复用完整渲染栈** | 上传隐藏窗不需要完整 UI，却仍单独开启完整 Chromium renderer（常 80–200MB+ 量级） |
...
| **6** | **Aria2 引擎** | 已改为按需启动；有下载或恢复任务时才占用进程 |
| **7** | **UI 依赖过重** | 全量 Arco CSS/组件 + 额外 Ant Design 局部 |
| **8** | **开发模式放大** | DevTools 自动打开、crx 扩展、Vite 未压缩 — 测内存勿当生产 |

**粗估空闲**：主进程 + 主窗 / 上传窗 renderer；无下载时不再常驻 `aria2c.exe`。开发模式的 DevTools、扩展和 Vite 仍会放大占用。
**不是某一个库单独导致**，主要仍是 **完整上传窗 × 重 UI 栈 × 预览复制窗**。
...
任务管理器 / Process Explorer：
  Mnemo.exe (main)
  Mnemo.exe (gpu)
  Mnemo.exe (renderer) × N   ← 空闲仍以 N=1 为目标
  aria2c.exe                 ← 下载或恢复任务时才出现
```
...
8. 任务列表虚拟滚动 + 历史分页
9. aria2 **懒启动**：已完成
10. 清理 `rss` / `reedy` / `book-*` / 死 `module/*`：已完成审计与移除

### 验证目标

下载空窗口已删除；继续将上传工作窗轻入口化后，空闲 renderer 可由 **2 → 1**，常驻内存仍有显著下降空间（视机器与任务量而定）。
...
| **内存主因** | **主窗与上传窗均跑完整 Vue+全量 Arco** + **预览复制整窗**；Aria2 已按需启动 |
