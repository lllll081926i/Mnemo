...
onMounted(() => {
  ticker = setInterval(() => {
    if (started.value && playing.value && duration.value > 0) {
      ...
      if (getPrefs().autoCloseOnEnd && position.value >= duration.value) {
        close()
      }
    }
  }, 500)
...
async function startPlayback() {
  ...
  try {
    GetVideoPreview(props.account.user_id, props.account.drive_id, props.file.file_id)
      .then((p) => { if (p && p.duration && !unmounted) duration.value = p.duration })
      .catch(() => {})
    ...
    if (!st || st.playbackResume !== false) {
      ...
      if (cursor && cursor > 5 && !unmounted) {
        await SeekPlayer(cursor).catch(() => {})
        position.value = cursor
      }
    }
  } catch (e) {
    if (!unmounted) error.value = String(e)
  } finally {
  ...
  <teleport to="body">
    <transition name="popover-zoom">
      <div class="player-panel">
        ...
        <div v-if="loading" class="pp-state"><span class="spin"></span>正在获取播放地址并启动播放器…</div>
