#!/bin/bash
# Bundle mnemo-mpv-texture.node + libmpv.dylib + transitive dylib deps
# into Mnemo's static macOS engine directory.
#
...
if [ ! -f "$NODE_FILE" ]; then
  ...
  echo "[bundle] run: cd native/mnemo-mpv-texture && pnpm install && pnpm run build" >&2
  exit 1
fi
...
cp "$NODE_FILE" "$OUTPUT_DIR/mnemo-mpv-texture.node"
cp "$LIBMPV_SOURCE" "$OUTPUT_DIR/libmpv.dylib"
chmod 755 "$OUTPUT_DIR/mpv_texture.node" "$OUTPUT_DIR/mnemo-mpv-texture.node" "$OUTPUT_DIR/libmpv.dylib"
...
install_name_tool -change "@rpath/libmpv.dylib" "@loader_path/libmpv.dylib" "$OUTPUT_DIR/mnemo-mpv-texture.node" 2>/dev/null || true
install_name_tool -change "$LIBMPV_SOURCE" "@loader_path/libmpv.dylib" "$OUTPUT_DIR/mnemo-mpv-texture.node" 2>/dev/null || true
otool -L "$OUTPUT_DIR/mpv_texture.node" | tail -n +2 | awk '{print $1}' | while read -r dep; do
  case "$(basename "$dep")" in
    libmpv*.dylib)
      ...
      install_name_tool -change "$dep" "@loader_path/libmpv.dylib" "$OUTPUT_DIR/mnemo-mpv-texture.node" 2>/dev/null || true
      ;;
  esac
done
