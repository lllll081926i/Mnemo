...
macOS-only N-API addon scaffold for Mnemo embedded MPV.
...
- The default stub backend has been verified to build into `build/Release/mnemo-mpv-texture.node` and export the expected JavaScript API shape.
- `mpvTexture.load()` currently rejects with `mnemo-mpv-texture libmpv backend is not implemented yet`.
...
npm_config_devdir=/private/tmp/mnemo-node-gyp-cache \
...
- copies `build/Release/mnemo-mpv-texture.node`;
...
This copies the default stub `.node` to `static/engine/darwin/<arch>/mpv-texture/mnemo-mpv-texture.node`, the same dev resource path used by `electron/main/mpv/embeddedMpvNativeAddon.ts`. The copied stub is gitignored because it cannot render video.
