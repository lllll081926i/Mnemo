...
func TestSanitizeTextKeepsQuotedURLBoundary(t *testing.T) {
	input := `Post "https://api.dropboxapi.com/oauth2/token": dial tcp`
	if got := sanitizeText(input); got != input {
		t.Fatalf("sanitizeText() = %q, want %q", got, input)
	}
}
